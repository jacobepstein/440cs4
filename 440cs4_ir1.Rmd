...
olp_norep_sd<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){sqrt(var(x[treat_norep]))/treat_norep_size+var(x[treat_norep])/control_norep_size})

match11_d<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){abs(mean(x[treat_norep])-mean(x[control_norep]))})
...
olp_rep_sd<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){sqrt(var(x[treat_rep]))/treat_rep_size+var(x[treat_rep])/control_rep_size})

match15_d<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){abs(mean(x[treat_rep])-mean(x[control_rep]))})
match15_asd<-match15_d/olp_rep_sd

#balance check for weighting
ipw_data<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){abs(mean(x[treat_norep])-sum(x[-treat_norep]*olp.debit$ipw[-treat_norep])/sum(olp.debit$ipw[-treat_norep]))})
