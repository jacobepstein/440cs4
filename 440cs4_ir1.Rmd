...
ATE_match_norep<-Match(Y=olp.debit$spending1998,Tr=olp.debit$debit_card1998,estimand="ATE",
                      X=data.matrix(olp.debit[,-c(1,2,16)]),M=1,Weight=1) #Tr not 1995?
...
ATE_match_rep<-Match(Y=olp.debit$spending1998,Tr=olp.debit$debit_card1998,estimand="ATE",
                      X=data.matrix(olp.debit[,-c(1,2,16)]),M=5,Weight=1) #^
...
#balance check
treat=olp.debit%>%filter(debit_card1998==TRUE)
treat_size=olp.debit%>%filter(debit_card1998==TRUE)%>%nrow()
control=olp.debit%>%filter(debit_card1998==FALSE) #or is it ATT_match_rep$index.treated?
control_size=olp.debit%>%filter(debit_card1998==FALSE)%>%nrow()
...
olp_sd<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){sqrt(var(x[treat]))/treat_size+var(x[treat])/control_size}) #x[treat] not working becasuse x is list... the output of x[treat] == x%>%filter(debit_card1998==TRUE) but the tidyverse version doesnt work...

#matching #norep and rep?
match_d<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){abs(mean(x[treat])-mean(x[control]))}) #^
match_asd<-match_d/olp_sd

#weighting
ipw_data<-apply(data.matrix(olp.debit[,-c(1,2,16)]),MARGIN=2,FUN=function(x){abs(mean(x[treat])-sum(x[-treat]*ipw[-treat])/sum(ipw[-treat]))}) #^
weight_asd<-ipw_data/olp_sd

#asd comparison boxplot
boxplot(ori_asd,match_asd,weight_asdmain="ASD for Different Methods",names=c("original","matching","weighting"),ylab="Abs Standardized Difference")
```
