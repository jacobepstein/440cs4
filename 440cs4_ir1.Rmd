...
ATE_matching_m<-Match(Y=olp.debit$spending1998,Tr=olp.debit$debit_card1998,estimand = "ATE",
                  X=data.matrix(olp.debit[,-c(1,2,16:21)]),M=6,Weight = 1)
...
ipw_bl_data<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x){abs(mean(x[treated])-sum(x[-treated]*olp.debit$ipw_bl[-treated])/sum(olp.debit$ipw_bl[-treated]))})

weight_bl_asd<-ipw_bl_data/olp_sd

ipw_rf_data<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x){abs(mean(x[treated])-sum(x[-treated]*olp.debit$ipw_rf[-treated])/sum(olp.debit$ipw_rf[-treated]))})

weight_rf_asd<-ipw_rf_data/olp_sd

ipw_CART_data<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x){abs(mean(x[treated])-sum(x[-treated]*olp.debit$ipw_CART[-treated])/sum(olp.debit$ipw_CART[-treated]))})
...
##MATCHING
match_d<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x)
...
match_asd_3<-match_d_3/olp_sd

pdf("asd_compare.pdf",width=8,height=6)
boxplot(ori_asd, match_asd, match_asd_1, match_asd_3, weight_bl_asd, weight_rf_asd, weight_CART_asd, main="ASD for Different Methods",names=c("original", "1:6M", "1:1M", "1:3M", "Logistic Wt","RF Wt","CART Wt"),ylab="Abs Standardized Difference")
dev.off()
