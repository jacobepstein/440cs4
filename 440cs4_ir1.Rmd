...
boxplot(ori_asd, debit.basicLog_weight$Balance$Diff.Adj, debit.randomForest_weight$Balance$Diff.Adj, debit.CART_weight$Balance$Diff.Adj, main="ASD for Different Methods",names=c("Original", "Logistic Weighting","Random Forest","CART Weighting"),ylab="Abs Standardized Difference")

boxplot(debit.basicLog_weight$Balance$Diff.Adj, debit.randomForest_weight$Balance$Diff.Adj, debit.CART_weight$Balance$Diff.Adj, main="ASD for Different Weighting Methods",names=c("Basic Logistic","Random Forest","CART"),ylab="Abs Standardized Difference")
```
...
ATE_match_norep<-Match(Y=olp.debit$spending1998,Tr=olp.debit$debit_card1998,estimand="ATE",
                      X=data.matrix(olp.debit[,-c(1,2,16:21)]),M=1,Weight=1) #Tr not 1995?
...
olp_norep_sd<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x){sqrt(var(x[treat_norep]))/treat_norep_size+var(x[treat_norep])/control_norep_size})

ipw_bl_data<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x){abs(mean(x[treat_norep])-sum(x[-treat_norep]*olp.debit$ipw_bl[-treat_norep])/sum(olp.debit$ipw_bl[-treat_norep]))})

weight_bl_asd<-ipw_bl_data/olp_norep_sd

ipw_rf_data<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x){abs(mean(x[treat_norep])-sum(x[-treat_norep]*olp.debit$ipw_rf[-treat_norep])/sum(olp.debit$ipw_rf[-treat_norep]))})

weight_rf_asd<-ipw_rf_data/olp_norep_sd

ipw_CART_data<-apply(data.matrix(olp.debit[,-c(1,2,16:21)]),MARGIN=2,FUN=function(x){abs(mean(x[treat_norep])-sum(x[-treat_norep]*olp.debit$ipw_CART[-treat_norep])/sum(olp.debit$ipw_CART[-treat_norep]))})

weight_CART_asd<-ipw_CART_data/olp_norep_sd
