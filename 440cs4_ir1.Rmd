...
first_model = glm(outcomeformula(), family = binomial(link="logit"), data = debit)
...
control<-ATT_matching_m$index.control ## Is there a reason to not take unique here
...
control_Y = controls$debit_card1998

treats = olp.debit[treated,]
treat_X = treats %>% dplyr::select(-spending1998, -debit_card1998)
treat_Y = treats$debit_card1998

model_control_m<-glm(outcomeformula(), controls, family=binomial(link="logit"))
model_treated_m<-glm(outcomeformula(), treats, family=binomial(link="logit"))
...
{
  return(debit_card1998 ~ s(spending1995) + s(num_of_earners) + average_age + s(family_size) + geograph_area + num_of_inhabitants + householder_age + s(householder_education) + s(num_of_banks) + s(interest_rate) + s(income) + s(wealth) + s(cash_inventory))
}
