...
#weight
debit.weighted=bal.tab(olp.debit[,-c(1,2,16)],treat=olp.debit$debit_card1998,method="weighting", weights=olp.debit$ipw,estimand="ATT")
...
ATT_mix = mean(olp.debit[ATT_match_rep$index.treated, "spending1998"]) - sum(olp.debit[ATT_match_rep$index.control, "spending1998"] + mu2 - mu1)/(6*treat_rep_size)
...
ATE_mix = ATE_match_rep$est - (-sum(mu3) + sum(mu4) - sum(mu5) + sum(mu6))/(6*dim(olp.debit)[1])
...
ATT_mix=mean(olp.debit %>% filter(debit_card1998))-sum(olp.debit[ATT_match_rep$index.control,"spending1998"]+mu2-mu1)/(6*treat_rep_size)
ATT_mix
