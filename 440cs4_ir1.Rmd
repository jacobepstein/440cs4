...
b<-hist(debit.ps$ps[which(debit.ps$debit_card1998==1)],breaks=50,add=TRUE)
...
```
We calculate propensity scores using general logistic regression. We then exclude the samples in the nonoverlapping region. One can see from the histogram that there is significant overlap between the propensity scores of the control and treatment groups.
...
#weight
debit.weighted=bal.tab(olp.debit[,-c(1,2,16, 17)],treat=olp.debit$debit_card1998,method="weighting", weights=olp.debit$ipw,estimand="ATT")
...
From our model, we can conclude that having a debit card caused the households with debit cards to increase their spending by about $460 per month. 
