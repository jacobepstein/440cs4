...
xtable(rbind(c(ATE_ipw_bl,ATE_DR,ATE_mix,ATE_mix_boost),
c(ATE_SD_IPW,ATE_SD_DR,ATE_SD_Mix,ATE_SD_Mix_boost),
c(ATT_ipw_bl,ATT_DR,ATT_mix,ATT_mix_boost),
c(ATT_SD_IPW,ATT_SD_DR,ATT_SD_Mix,ATT_SD_Mix_boost)),digits=4)

# Get only those that worked
ATT_SD_IPW<-ATT_SD_IPW[ATT_SD_IPW != 0]
ATT_SD_DR<-ATT_SD_IPW[ATT_SD_IPW != 0]
ATT_SD_Mix<-ATT_SD_IPW[ATT_SD_IPW != 0]
ATT_SD_Mix_boost<-ATT_SD_IPW[ATT_SD_IPW != 0]

ATT = c(ATT_ipw_bl,ATT_DR,ATT_mix,ATT_mix_boost) %>%unlist()
ATT_SD = c(ATT_SD_IPW,ATT_SD_DR,ATT_SD_Mix,ATT_SD_Mix_boost) %>%unlist()
AOT = debit %>% filter(debit_card1998 == 1) %>% dplyr::select(spending1998) %>% summarize(mean = mean(spending1998))
AOT_over_AOT = ATT / AOT %>% unlist %>%as.numeric()
