...
We move forward with attempts to balance the covariates(matching, weighting, propensity scores) as these plots illustrate significant differences in the covariates between the spending1998 and control groups. 
...
legend("topright",legend=c("Control Group","debit_card1998 Group"),col=c("red","blue"),pch=15,cex=0.7)
dev.off()
...
loveplot(olp.debit[,-c(1,2,16:21)],ATE_matching_1$index.treated, ATT_matching_1$index.control, .1)
...
#Double Robustness
#ATT DR
ATT_DR<-mean(olp.debit["spending1998","debit_card1998"])-sum((olp.debit$debit_card1998*(1-olp.debit$spending1998)*
                                  olp.debit$ps+predict(model_control,newdata=olp.debit,type="response")*
                                               (olp.debit$spending1998-olp.debit$ps))/(1-olp.debit$ps))/treat_size
#ATE DR
ATE_DR<-mean(olp.debit$spending1998*olp.debit$debit_card1998/olp.debit$ps_basicLog)-
    mean((olp.debit$spending1998-olp.debit$ps_basicLog)*predict(model_treated,newdata=olp.debit,type="response")/olp.debit$ps_basicLog)-
    mean((1-olp.debit$spending1998)*olp.debit$debit_card1998/(1-olp.debit$ps_basicLog))-
    mean((olp.debit$spending1998-olp.debit$ps_basicLog)*predict(model_control,newdata=olp.debit,type="response")/(1-olp.debit$ps_basicLog))
```
![](overlap_check.png)

We calculate propensity scores using general logistic regression. We then exclude the samples in the nonoverlapping region. One can see from the histogram that there is significant overlap between the propensity scores of the control and debit_card1998 groups; however, we expect this can be improved upon.

![](ASD_compare.png)

Moving forward with 1:1 matching, we see that the absolute mean differences are improved significantly across almost all covariates.
...
#ATT_mix = mean(unlist(olp.debit[ATT_matching_1$index.treated, "spending1998"])) - #sum(olp.debit[ATT_matching_1$index.control, "spending1998"] + mu2 - mu1)/(6*treat_size1)


ATT_mix = mean(treat_Y) - sum(control_Y + mu2 - mu1)/(6*treat_size1)
...
ATE_mix = ATE_matching_1$est - (-sum(mu3) + sum(mu4) - sum(mu5) + sum(mu6))/(6*dim(olp.debit)[1])
...
ATT_mix_boost=mean(olp.debit %>% filter(debit_card1998 == 1) %>% dplyr::select(spending1998) %>% unlist())-sum(olp.debit[ATT_matching_1$index.control,"spending1998"]+mu2-mu1)/(6*treat_size1)
...
ATE_mix_boost=ATE_matching_1$est-(-sum(mu3)+sum(mu4)-sum(mu5)+sum(mu6))/(6*dim(olp.debit)[1])
...
ATT_mix_forest=mean(olp.debit %>% filter(debit_card1998 == 1) %>% dplyr::select(spending1998) %>% unlist())-sum(olp.debit[ATT_matching_1$index.control,"spending1998"]+mu2-mu1)/(6*treat_size1)
...
ATE_mix_forest=ATE_matching_1$est-(-sum(mu3)+sum(mu4)-sum(mu5)+sum(mu6))/(6*dim(olp.debit)[1])

data.frame(ATT_mix,
      ATE_mix,
      ATT_mix_boost, 
      ATE_mix_boost, ATT_mix_forest, ATE_mix_forest)
...
#Direct regression
      ...
      #control
      b_control<-glm(debit_card1998 ~ spending1995 + num_of_earners + average_age + family_size + geograph_area +  num_of_inhabitants + householder_age + householder_education + num_of_banks + interest_rate + income + wealth + cash_inventory, family = binomial(link="logit"), data = bdata %>% filter(debit_card1998 == 0))
      
      #treated
      b_treated<-glm(debit_card1998 ~ spending1995 + num_of_earners + average_age + family_size + geograph_area +  num_of_inhabitants + householder_age + householder_education + num_of_banks + interest_rate + income + wealth + cash_inventory, family = binomial(link="logit"), data = bdata %>% filter(debit_card1998 == 1))
      
      #ATT calculation:
      b_reg1[i]<-sum(bdata[bdata$debit_card1998==1,"spending1998"]-predict(b_control,newdata=bdata[bdata$debit_card1998==1,],type="response"))/sum(bdata$debit_card1998)
      
      #ATE Calculation:
      b_reg2[i]<-(sum(subset(bdata,debit_card1998==1)$spending1998-predict(b_control,newdata = subset(bdata,debit_card1998==1),type="response"))-
          sum(subset(bdata,debit_card1998==0)$spending1998-predict(b_treated,newdata = subset(bdata,debit_card1998==0),type="response")))/dim(bdata)[1]
...
#Matching/Mix
      ...
      b_mix1[i]=mean(bdata[treated,]$spending1998)-sum(bdata[control,"spending1998"]+mu2-mu1)/(6*treat_size)
...
#IPW
      ...
      b_IPW1[i]<-mean(bdata[btreated,"spending1998"])-sum(bdata[-btreated,"spending1998"]*bipw[-btreated])/sum(bipw[-btreated])
...
#xtable(rbind(c(ATE_1,ATE_mix,ATE_ipw_bl,ATE_DR), #ATE_ipw_bl is a placeholder check this, no ATE_DR
